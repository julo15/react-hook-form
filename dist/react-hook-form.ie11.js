...
var appendId = function (value) { return (__assign(__assign({}, (isObject(value) ? value : { value: value })), { id: value.id || generateId() })); };
...
var breakFieldArrayName = function (name, searchName) {
    var prefix = searchName + "[";
    if (!name.startsWith(prefix)) {
        return;
    }
    var startIndex = prefix.length;
    var endIndex = name.indexOf(']', startIndex);
    var index = Number(name.substring(startIndex, endIndex));
    var suffix = name.substring(endIndex);
    return {
        index: index,
        prefix: prefix,
        suffix: suffix,
